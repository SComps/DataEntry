...
Imports System.Collections.Generic
    ...
    Public Class CodeGenerator
        ...
        Private Sub WriteFormatHelper(dir As String, ns As String)
            ...
            sb.AppendLine("            End If")
            sb.AppendLine("            ' Strip literal characters embedded in the mask from the input so that")
            sb.AppendLine("            ' pre-formatted values (e.g. ""800-867-5309"") work like raw digits.")
            ...
            sb.AppendLine("        End Function")
            sb.AppendLine()
            sb.AppendLine("        ' Strips non-data characters from raw based on what the mask's placeholder types accept.")
            sb.AppendLine("        ' This normalises pre-formatted input: ""800-867-5309"" and ""(800)867-5309"" both")
            sb.AppendLine("        ' collapse to ""8008675309"" for a digit-only mask before ApplyMask runs.")
            sb.AppendLine("        Public Function StripLiterals(raw As String, mask As String) As String")
            sb.AppendLine("            ' Only normalise when the mask has embedded literal separators (\x escapes).")
            sb.AppendLine("            ' A mask with no literals doesn't need stripping — placeholders already")
            sb.AppendLine("            ' handle unexpected chars (9 emits a space for non-digits, etc.).")
            sb.AppendLine("            If Not mask.Contains(""\"") Then Return raw")
            sb.AppendLine("            ' Determine which placeholder types appear in the mask.")
            ...
            sb.AppendLine("                    mi += 2 : Continue Do  ' escaped literal — skip")
            sb.AppendLine("                End If")
